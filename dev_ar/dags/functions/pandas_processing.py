...
def common_tasks(df, path_to_data_docker):
    ...
    try:
        ...
        df_loc_pc = pd.read_csv(
            Path(path_to_data_docker, 'codigos_postales').with_suffix('.csv')
        )
    except:
...
def transform_univ_kennedy(path_to_data_docker, univ_kennedy_file_name):
    ...
    uni_kennedy_name = Path(path_to_data_docker, univ_kennedy_file_name).with_suffix('.csv')
    try:
        # df = pd.read_csv(f"{path_to_data_docker}{univ_kennedy_file_name}.csv")
        df = pd.read_csv(uni_kennedy_name)
    except:
    ...
    try:
        ...
        df.to_csv(Path(uni_kennedy_name).with_suffix('.txt'), index=False, sep=",")
...
def transform_facu_latinoamericana(path_to_data_docker, facu_latinoamericana_file_name):
    ...
    uni_latam_name = Path(path_to_data_docker, facu_latinoamericana_file_name).with_suffix('.csv')
    try:
        # df = pd.read_csv(f"{path_to_data_docker}{facu_latinoamericana_file_name}.csv")
        df = pd.read_csv(uni_latam_name)
    except:
    ...
    try:
        ...
        df.to_csv(Path(uni_latam_name).with_suffix('.txt'), index=False, sep=",")
